# Transformers PR 42697: `src/transformers/integrations/moe.py`

Source PR: https://github.com/huggingface/transformers/pull/42697

Source file at PR head commit:
https://raw.githubusercontent.com/huggingface/transformers/2ebaba2d481af235a20f012572c41e558cb1e908/src/transformers/integrations/moe.py

Fetched: 2026-05-03

Study focus:

- `batched_mm_experts_forward` expands each token's top-k expert assignments into
  per-selected-sample rows, selects expert weights per sample, then uses
  `torch.bmm`.
- `grouped_mm_experts_forward` performs the same logical work, but sorts samples
  by expert, computes per-expert offsets, calls `torch._grouped_mm`, then restores
  original order before `index_add_`.
- `use_experts_implementation` is the model-facing decorator that swaps an
  expert module's original eager `forward` for a registered backend.
...
def grouped_mm_experts_forward(
    ...
    # This uses histc instead of bincount to avoid cuda graph issues
    # (grouped_mm_experts_forward still fails with cuda graphs because of _grouped_mm internals).
    ...
    if hasattr(self, "gate_up_proj_bias") and self.gate_up_proj_bias is not None:
        # Bias support in the grouped_mm call was not fully available here.
